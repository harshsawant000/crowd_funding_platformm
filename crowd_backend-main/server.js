...
// Ensure routes object is properly structured
if (routes) {
  if (routes.campaign) app.use("/api/campaign", routes.campaign);
  if (routes.user) app.use("/api/user", routes.user);
  if (routes.payment) app.use("/api/donate", routes.payment);
  if (routes.donation) app.use("/api/donation", routes.donation);
  if (routes.query) app.use("/api/query", routes.query);
} else {
